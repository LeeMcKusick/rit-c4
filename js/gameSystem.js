	...
	**********/
		...
		$.ajax( { 
			...
			success: function( jsonText ) {
				if (jsonText != 'null') { 
					...
					if(repeatLoop) {
						setTimeout("GameSystem.checkStatus()", 2500);
					}
				}
			}
		});
	...
	playPiece: function (id) {
		if (whoseTurn == name) {
			...
			if (firstDroppable) {
				//alert ("cell_" + firstDroppable.col + firstDroppable.row);
				//document.getElementById("cell_" + firstDroppable.col + firstDroppable.row).style.fill = "#FF0000";
				//alert( firstDroppable.getCenterX() + ", " + firstDroppable.getCenterY())
				/*var cir = document.createElementNS(svgns, 'circle');
				cir.setAttributeNS(null, 'cx', firstDroppable.getCenterX() );
				cir.setAttributeNS(null, 'cy', firstDroppable.getCenterY() );
				cir.setAttributeNS(null, 'r', '20px');
				cir.setAttributeNS(null, 'fill', color);
				document.getElementsByTagName('svg')[0].appendChild(cir);
				var cellAbove = getNextCellInColumn(firstDroppable.id);
				firstDroppable.droppable = false;
				cellAbove.droppable = true;*/
				$.ajax({
					url: this.AJAX_URL,
					data: "call=insertpiece&gameID=" + gameID + "&column=" + firstDroppable.col,
					success: function ( jsonText ) {
						if (jsonText == "Sent") GameSystem.checkStatus();
						GameSystem.getBoard();
					}
				});
			} else {
	...
	chatFetch: function () {
		...
		setTimeout("GameSystem.chatFetch()", this.CHAT_FETCH_INTERVAL);
